        ...
        <v-list>
          <v-list-item v-for="(user, index) in users" :key="user.user_id" class="user-card">
            <v-card :class="{ 'highest-points': index === 0 }">
              ...
              <v-card-subtitle>Points: {{ user.points }}</v-card-subtitle>
            </v-card>
          </v-list-item>
        </v-list>
      </v-col>
...
export default {
  ...
  methods: {
    async fetchUsers() {
      try {
        const response = await fetch('http://localhost:3000/api/users-with-points');
...
.highest-points {
  background-color: rgb(41, 136, 41); /* Guld farve for brugeren med de højeste point */
}
</style>